...
> 참고. 빈약한 도메인과 리치 도메인
### 빈약한 도메인
- getter, setter만 있는 모델

### 리치 도메인
- 각 모델이 도메인을 설명하는 모델이 되어야 한다.
- 객체의 인터페이스를 통해 기능에 접근할 수 있게 만들어진다.
- 도메인을 설명하기 위해서는 행동을 명확하게 정의하고, 관심사가 적절하게 분리되어야 한다. (각 객체가 하고자하는 목표, 초점에 따라 적절하게 나눠져야 한다는 의미)
- 도메인 전문가 및 개발자와 협의 아키텍처를 구조화하고 분리해야 한다.
