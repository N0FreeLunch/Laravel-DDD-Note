...
여기서의 전략 패턴은 GoF 디자인 패턴의 전략 패턴을 의미하지 않는다. DDD의 전략 패턴은 비즈니스 규칙 및 알고리즘을 캡슐화하여 표현하는 것을 뜻한다.

하지만 GoF의 전략 패턴은 DDD의 전략 패턴을 이해하는데 참고할만한데, 디자인 패턴의 전략 패턴은 동일한 인터페이스의 구현체인 객체를 주입해서 교체하는 형태로 만들어 필요에 따라 바뀔 수 있는 로직을 교체하는 방식을 의미한다.

DDD에서의 전략패턴은 도메인의 의미적인 측면으로 이해해야 하는데, 변하지 않는 비즈니스 규칙과 변할 수 있는 규칙을 나누어 변하지 않는 비즈니스 규칙에 변하는 비즈니스 규칙을 교체할 수 있는 형태로 추상화하는 것을 의미한다.

전략 패턴은 변하는 도메인 로직을 교체할 수 있도록 만드는 것이지만, 기존 로직에 옵션으로 추가하는 개념을 설계할 때도 유용한 방법이다.

### 어뎁터 패턴

여기서의 어뎁터 패턴은 GoF 디자인 패턴의 어뎁터 패턴을 의미하지 않는다. DDD의 어뎁터 패턴은 서로 다른 레이어 또는 도메인이 가진 인터페이스를 서로 연결하기 위한 패턴이다.

하지만 GoF의 어뎁터 패턴은 DDD의 어뎁터 패턴을 이해하는데 참고할만한데, 디자인 패턴의 어뎁터 패턴은 서로 다른 인터페이스를 가진 객체를 각 객체를 변경 손상 시키지 않고 둘을 잇는 객체를 만들어 연결하는 역할을 한다.

DDD에서 어뎁터 패턴은 계층적인 의미와 도메인의 경계의 의미 두 가지로 바라볼 수 있는데, 비즈니스 로직을 담은 계층과 그렇지 않은 계층 간의 인터페이스는 나눠져 있고 이를 연결하기 위한 것이 계층적인 연결을 위한 어뎁터 패턴이며, 도메인 계층에서 각 도메인 간의 인터페이스를 통합 연결하여 필요한 기능을 만들어내는 것이 도메인 경계에서의 어뎁터 패턴이다.

어뎁터 패턴은 모놀리식 DDD에서는 하나의 프레임워크 내에서의 코드들간의 통합을 의미하기도 하지만, 마이크로 서비스 아키텍처에서는 서로 다른 시스템 또는 서로 다른 프로그래밍 언어로 된 프로그램을 통합하기 위한 패턴을 의미하기도 한다.
...
- 도메인 모델(실제의 비즈니스 로직을 모델화 하는 것)과 응용 프로그램(프로그래머가 만든 소프트웨어) 간의 연결 역할을 한다. 그러니까 애플리케이션을 도메인 모델을 중심으로 설계할 수 있도록 도구들을 제공하는 기능들이 배치된 영역이다.
...
### Big Ball of Mud (BBOM)의 특징

#### 구조의 문제

- 무계획적인 구조화 (구조화를 하는데 일정한 룰이나 계획 없이 되는대로 구조화 하는 것)
- sloppy (sloppy : 엉성한 구조)로 규칙, 패턴, 로직의 흐름이 드러나지 않는 코드
- 도메인 로직과 기술 인프라 로직의 뒤섞임

#### 단기적인 해결책 의존

- 문제를 해결하기 위한 임시 뗌빵식 로직 (duct-tape-and-baling-wire)
- 스파게티 코드로 이해를 어렵게 하는 것

#### 통제 불가능

- sprawling (불규칙하게 아무렇게나 만들어서 서비스 모델을 발전시키는 것)
- 규칙없는 성장과 반복의 징후가 만연되는 것

#### 개선 불가

- 시스템의 개선 및 변경시 예측 불가능에 따른 비용 산출 불가 및 높은 비용 소모
