...
- DTO를 사용하여 데이터를 영속성 계층의 데이터와 애플리케이션 계층의 데이터로 나누는 것은 동일한 데이터를 원본 데이터와 애플리케이션용 데이터 두 가지 형태로 나눠서 저장하고 있다. 
- DTO를 영속성 계층 데이터와 별로도 애플리케이션 계층의 데이터로 표현하기 위한 용도로 사용하기 위해서는 원본 데이터를 저장하는 DTO와 영속성 계층의 데이터로 표현하기 위한 DTO를 만들어서 사용하게 된다. 이는 데이터를 2중으로 다루게 되기 때문에 나쁜 디자인으로 볼 수 있다.
- 그리고 영속성 계층의 데이터를 저장하는 DTO를 통해서 애플리케이션 계층의 데이터를 저장하기 위한 DTO를 만드는 transfomer을 요구한다.
