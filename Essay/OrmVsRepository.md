...
실제 환경과 최대한 동일한 환경에서 테스트 코드를 동작시키기 위해서는 작성한 코드 뿐만 아니라, 사용하고 있는 라이브러리와 프레임워크의 코드도 함께 실행해야 한다. 어떤 코드를 작성하고 코드의 동작을 실행하기 위해서 프레임워크와 작성된 코드를 모두 빌드하는 것은 많은 시간이 걸린다. 실제 코드를 개발할 때는 동작을 확인하는 과정이 반복되는데 코드의 실행 시간에 적어도 수십초의 시간이 걸리게 되면 그 만큼 생산성이 떨어지게 된다. 코드 규모가 커질수록 컴파일 시간이 더 오래 걸리기 때문에 프로젝트 규모가 커질수록 한 번 테스트하기 위해 걸리는 시간이 늘어날 수 있다.
...
도메인 주도 설계에서 전술적 패턴은 코딩의 패턴에 대해 알려 준다. 도메인주도 설계는 적절한 단위로 도메인을 나누고 각 도메인에 레포지토리 클래스, 엔터티 클래스, DTO 클래스, VO 클래스를 만들고 이들을 사용하여 로직을 구성한다. 특히 레포지토리 패턴은 데이터베이스의 데이터를 가져와서 데이터를 객체 또는 데이터 구조에 담아내는 역할을 한다. 이러한 전술적 패턴은 하나의 도메인을 구성하는데 필요한 데이터들을 가져오기 위해 만들어지며, 각각의 도메인은 서로의 도메인에 맞는 전술적 패턴을 사용하도록 하며 서로 다른 도메인에서 단순히 필요한 데이터를 가져올 수 있다고 해서 다른 도메인의 레포지토리 또는 엔터티를 가져오지 않도록 도메인간의 의존성을 분리하는 스타일로 만든다.
...
인메모리 디비를 지원하는 데이터베이스는 코드 실행에 필요한 모든 리소스를 메모리에 올려 mysql과 sqlite가 있다. mysql의 경우 설치에 많은 리소스가 필요하기 때문에 테스트용 전용 서버를 구축해야 한다. sqlite의 경우 빠르게 설치하고 실행할 수 있기 때문에 테스트용 인메모리 디비로는 sqlite를 주로 사용한다. sqlite는 여러 동시적인 엑세스가 어렵다는 문제점이 있어, 싫제 프로덕션의 데이터베이스로는 사용되지 않는다.
...
로우 쿼리를 사용하면, 각 데이터베이스에서 제공하는 쿼리 최적화를 이용할 수 있다는 장점이 있다. 하지만, 어떤 데이터베이스에서 제공하는 특별한 문법을 사용한 쿼리로 최적화를 할 수록 테스트 디비에서 동일한 동작을 수행하기 위한 쿼리를 작성하므로 두 개의 쿼리를 작성해야 한다. 코드는 검증되어야 하므로 가능한 동일한 코드로 테스트를 수행하는 것이 좋다. 따라서 최적화를 기본적인 코딩 스타일로 사용하기 보다는 꼭 필요한 부분에만 사용하고, 일부러 최적화를 위한 특정 데이터베이스에 의존적인 방식은 피하도록 하자.
...
라라벨에서 제공하는 EloquentORM은 사양패턴을 구성하기에 아주 좋다. 라라벨의 쿼리 빌더가 제공하는 인터페이스에 따라 코드를 구성하는 것으로 다양한 데이터베이스에서 동일하게 동작하는 쿼리를 만들 수 있다는 장점이 있다. 주로 동적 타이핑 언어의 ORM은 raw 쿼리에 의존하지 않는 쿼리 빌더를 제공하며, JAVA의 JPA나 C#의 엔터티 프레임워크 등은 로우쿼리에 의존하는 코드를 불가피하게 만들어내는 경우가 많다.

#### 즉시로딩(eager loading)의 사양패턴

라라벨의 엘로퀀트는 아주 특별한 점이 있는데, 즉시 로딩(eager loading)에도 사양패턴을 적용할 수 있다는 점이다. 각각의 모델에는 연관 모델에 연결될 수 있는 기능을 제공한다. 이 연관 모델에 연결하는 기능을 통해서 ORM은 즉시 로딩을 할 수 있게 만들어 주는데, 연관 모델의 연관 모델을 즉시 로딩하기 위해서는 `$modelA->with('modelB.modelC')`를 사양패턴으로 만들어 `$modelA->withModelC()`의 코드를 만들 수 있을 것이다. 하지만 모델 A에 직접 연결되어 있는 것은 모델 B인에 모델 C를 연결하는 것이 어색하다. 가능한 모델에는 각 테이블이 담당하는 기능만을 담는 것이 좋은데 직접적인 연관성이 없는 대상의 모델도 연결해야 하기 때문에 모델 A가 담당해야 하는 역할을 넘어서는 코드를 만들게 되므로 어색하다. 라라벨은 `$modelA->with('modelB', function ($relation) {$relation->with('modelC')})`방식의 코딩 스타일을 지원한다. 그러면 모델 A의 인스턴스 안에 모델 B의 인스턴스가 즉시 로딩 되어 있고, 모델 B의 인스턴스 안에 모델 C가 즉시로딩 되어진다. 이를 사양 패턴화 하면 `$modelA->withModelB(function ($relation) {$relation->withModelC(fn($relation) => $relation->query()->isSpecification()); })`의 코드를 만들 수 있다. 모델 A는 직접적인 연관성이 있는 모델 B만을 연결하면 되고, 모델 B는 직접적인 연관성이 있는 모델 C를 연결하여, 모델A에서 모델C를 즉시 로딩해서 접근할 수 있도록 기능을 제공하는 것이다.

엘로퀀트와 비슷한 레일즈의 엑티브레코드에서는 라라벨과 같은 사양 패턴화를 사용하자고 하기 어려운데, `ModelA.includes(:model_b: :model_c)`의 코드를 별도의 메소드로 만들기 위해서는 모델A 클래스에 모델C의 코드를 넣을 수 밖에 없게 되기 때문이다. 따라서 직접적인 연관 관계가 없는 대상도 모델에 추가해야 한다는 문제가 발생한다. 그래서 레일즈의 즉시 로딩은 별다른 사양패턴화 하지않고 쓰는 편이 낫다. 엘로퀀트의 경우 `$modelA->with('modelB.modelC')`으로 문자열로 연관 모델을 지정해 주어야 해서 메소드로 추상화하는 것이 좋지만, 레일즈의 경우 각 모델 클래스에서 정의된 심볼(`:model_b`, `:model_c`)을 사용하기 때문에 굳이 메소드로 굳이 레핑하지 않아도 된다. 물론 관계 모델 뿐만 아니라 다른 개념도 심볼로 키워드화 할 수 있기 때문에 연관 모델의 심볼이 무엇인지 정도는 파악하는 수고가 필요하다. 즉시 로딩을 할 때 라라벨 쪽이 좀 더 우아한 코드 스타일을 제공하는 것을 알 수 있다.
