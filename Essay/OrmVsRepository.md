...
엘로퀀트와 비슷한 레일즈의 엑티브레코드에서는 라라벨과 같은 사양 패턴화를 사용하자고 하기 어려운데, `ModelA.includes(:modelB: :modelC)`의 코드를 별도의 메소드로 만들기 위해서는 모델A 클래스에 모델C의 코드를 넣을 수 밖에 없게 되기 때문이다. 따라서 직접적인 연관 관계가 없는 대상도 모델에 추가해야 한다는 문제가 발생한다. 그래서 레일즈의 즉시 로딩은 별다른 사양패턴화 하지않고 쓰는 편이 낫다.
