...
## Fulfillment
- 이행(Fulfillment)의 정의 : (물류업, 유통업 등에서) 고객의 주문에 맞춰 물류센터에서 제품을 피킹, 포장, 배송까지의 과정 전반
- 주문의 바운디드 컨텍스트 내에서 Fulfillment에 대한 모든 구현을 하는 것이 아니라 주문 워크플로우를 사용하여 컨텍스트 스위칭을 통한 주문을 구성하는 것이 좋다.
- 이를 통해서 시스템의 객체 수가 적어지고(?) 디버깅 할 때 사람이 추적해야 할 항목이 적어진다.
...
- Product Model
- Product Tracking
- Quantity Tracking (Inventory)
- Product Tracking
- Location Code

## 제품 위치
- 문제 정의 : 창고에 있는 수천 개의 선반 중 제품을 찾는데 물리적으로 너무 많은 시간이 걸린다.
- 문제 해결 : 주어진 제품의 위치를 정확히 파악하여 제품을 골라 포장할 수 있도록 도와주는 솔루션을 만든다.
- 코드를 사용하지 않고 문제에 대한 솔루션을 제공하는 예를 만들어서 검토하는 과정이 필요하다.
...
### 솔루션
- 창고 위치에서 신발 한 켤레를 빠르게 식별할 수 있는 방법을 얻기 위해서는 제품의 앤티티에 제품의 위치를 나타내는 코드를 추가 해 줘야 함.
- 특정 신발을 찾는 것은 합리적인 시간 내에 창고 내에서 획득 할 수 있도록 분류하는 방식을 사용하자. 더 작고 관리하기 쉬운 작업 단위로 분해의 필요가 있다.
- 도메인 모델의 개념이 자연스럽게 분리되는 방식을 살펴보자 : 성별에 따른 신발의 구분 (남, 여, 남여공용)


### 나눌 수 있는 방법을 생각
- 남자 신발, 여자 신발, 남녀 공용의 세 가지로 나눌 수도 있으며
- 남자 신발, 여자 신발로 나누고 남자 신발 안에 남녀 공용 제품, 여자 신발 안에 남녀 공용 제품을 넣는 방식으로 나눌 수도 있다. 이 경우 공용 상품의 경우 보관 위치가 두 가지 나올 수 있다는 문제점이 생긴다.
