...
- 도메인 계층의 인터페이스를 다룬다.
- 리포지토리 계층을 담는 부분이다.
- 의존성 주입을 이용하여 IOC를 구현한다. IOC를 구현하게 되면 인프라 계층 아래 계층인 도메인 계층에 인터페이스를 제공할 수 있다.
- 하나의 클래스로부터 다수의 객체를 생성해 내는 펙토리 패턴을 사용하는 계층이다. 하나의 구성요소를 다양한 컨텍스트에서 객체로 불러와서 사용하는 계층으로 디비 모델 클래스로 여러 모델 객체를 생성한다던 하는 기능을 활용하는 계층이다. 인프라스트럭처 계층 뿐만 아니라 도메인 계층도 펙토리 패턴을 사용한다.
- 영속성 계층의 데이터를 조작하는 기능을 하는 데이터베이스 모델과 같은 기능을 정의하는 계층이다.
...
## 도메인레이어 (+ 모델 레이어)
### 앤티티
- 정보의 단위를 의미한다. 정보는 어떤 구분할 수 있는 속성들이 모여서 하나의 개념적인 단위를 창출한 것을 의미한다. 부품 A, 부품 B, 부품 C가 서로 모여 하나의 제품이 된다고 하자. 그럼 A, B, C는 부품의 속성이며 A, B, C가 서로 모여서 제품이란 단위를 형성을 할 때, 제품이 되면서 A, B, C가 가진 속성 이외의 추가적인 인터페이스를 제공하는 단위가 되는 것이 앤티티의 의미이다.
