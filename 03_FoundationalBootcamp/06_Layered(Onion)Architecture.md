...
## 애플리케이션 서비스
- 레이어의 역할보다는 비즈니스의 특정 작업이나 프로세스에 초점을 맞춘다.
- 클라이언트와 도메인 논리 사이의 중개자 역할을 한다.
- MVC 아키텍처의 C에 해당한다.
- 서비스의 작업을 패치(fetch) 클라이언트의 요청에 응답하기 위해 응답을 만들어 낸다.
- 리스너, 가입자, 기타 수신자를 보유함
- 컨트롤러는 애플리케이션 레이어의 핵심적인 특성

### 도메인 로직을 애플리케이션 레이어의 컨트롤러에 정의할 경우
- 비즈니스 로직의 실행 순서를 컨트롤 할 때 컨트롤러에서 컨트롤 하는 경우 도메인 모델이나 MVC의 모델을 컨트롤러 안에 코드로 작성할 경우가 있다. 이렇게 되면 모든 종류의 테스트가 어려워진다.
- 특히 모듈들을 단위 테스트 할 수 없는 문제가 생긴다.
