...
- 애플리케이션이라는 것은 유저가 어떤 처리를 하고 싶어 하는 것이다. 유저가 원하는 특정 인터페이스를 통해 처리 로직이 이뤄진다는 것은 컨텍스트를 형성한다. 이 컨텍스트를 이루기 위해 여러 모듈화된 동작들이 참여하여 한 단위의 비즈니스 로직을 구성한다. 컨텍스트를 나타내는 부분과 모듈화된 동작 단위들은 나눠진다. 이런 컨텍스트와 구성요소의 부니리가 도메인 모델의 핵심이다.
